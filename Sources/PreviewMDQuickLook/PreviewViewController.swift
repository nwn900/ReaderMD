    ...
    enum Error: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .missingResource(let name):
                "PreviewMD Quick Look is missing the renderer resource \(name)."
            case .invalidPayload:
                "PreviewMD couldn’t prepare the Markdown preview."
            case .webContentProcessTerminated:
          ...
          (function () {
            ...
            article.addEventListener("error", function (event) {
              ...
              placeholder.textContent =
                label + "local image is available after opening the document in PreviewMD";
              image.replaceWith(placeholder);
            }, true);
            window.previewmdRender(\(payload));
          })();
          </script>
        </body>
        </html>
