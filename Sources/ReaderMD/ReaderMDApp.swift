...
struct ReaderMDApp: App {
    ...
    var body: some Scene {
        Window("ReaderMD", id: MainWindow.id) {
            WorkspaceView()
        ...
        Window("About ReaderMD", id: AboutWindow.id) {
            AboutView()
                .previewWindowAppearance(state.theme)
        }
...
struct ReaderMDCommands: Commands {
    @ObservedObject var state: AppState
    @Environment(\.openWindow) private var openWindow

    var body: some Commands {
        CommandGroup(replacing: .appInfo) {
            Button("About ReaderMD") {
