...
final class EditorWebViewTests: XCTestCase, WKNavigationDelegate {
    ...
    func testWebViewPreparesPrintableContentAndPageBreaks() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            const layout = JSON.parse(
              await window.readermdPreparePDF(options, 515, 761)
            );
            return {
              layout: layout,
    ...
    func testRelativeAndAbsoluteLocalImagesDecodeAndRoundTrip() async throws {
        let folder = FileManager.default.temporaryDirectory
            .appendingPathComponent("ReaderMD-local-image-\(UUID().uuidString)")
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            return {
              ...
              originals: images.map((image) => image.dataset.readermdSource),
              widths: images.map((image) => image.naturalWidth),
              markdown: window.readermdSerializeEditor(),
            };
            """,
            contentWorld: .page
        )
    ...
    func testPortableClipboardPayloadReplacesRendererInternalsWithSemanticContent() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            return window.readermdBuildClipboardPayload();
            """,
            contentWorld: .page
        )
        ...
        XCTAssertTrue(html.contains("readermd-copy-asset:"))
    ...
    func testPortableClipboardPreservesSemanticHeadingLevels() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            selection.removeAllRanges();
            selection.addRange(range);
            return window.readermdBuildClipboardPayload().html;
            """,
            contentWorld: .page
        )
    ...
    func testClipboardAssetSnapshotStageContainsRenderedFormula() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            selection.removeAllRanges();
            selection.addRange(range);
            return window.readermdBuildClipboardPayload().assets[0];
            """,
            contentWorld: .page
        )
        ...
        let frameResult = try await webView.callAsyncJavaScript(
            """
            const frame = await window.readermdPrepareClipboardAssetSnapshot(
              markup,
              width,
              height
            );
    ...
    func testRichSelectionSerializesAsBoldMarkdown() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            return window.readermdFlushEditor();
            """,
            contentWorld: .page
        )
    ...
    func testRichFormattingInsideListPreservesOneMarkdownItem() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            return {
              markdown: window.readermdFlushEditor(),
              sourceStart: item.dataset.readermdSourceStart,
              sourceEnd: item.dataset.readermdSourceEnd,
            };
            """,
            contentWorld: .page
        )
    ...
    func testComplexRenderedObjectsRoundTripToMarkdown() async throws {
        ...
        let webView = try await makeEditor(markdown: markdown)
        let result = try await webView.callAsyncJavaScript(
            "return window.readermdSerializeEditor();",
            contentWorld: .page
        )
    ...
    func testVisualDiagramModelParsesAndSerializesCommonFlowcharts() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            const parsed = window.ReaderMDDiagramEditor.parse(source);
            return {
              ok: parsed.ok,
              ...
              serialized: parsed.ok
                ? window.ReaderMDDiagramEditor.serialize(parsed.model)
                : "",
            };
            """,
            contentWorld: .page
    ...
    func testAdvancedMermaidOpensInCodeModeWithoutRewritingSource() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            editor.querySelector("[data-diagram-cancel]").click();
            result.markdown = window.readermdFlushEditor();
            return result;
            """,
            contentWorld: .page
        )
    ...
    func testSplitSyncMapsSelectionsBetweenSourceAndRenderedText() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            const applied = window.readermdApplySplitSelection
            ...
            selection.removeAllRanges();
            selection.addRange(range);
            const previewToSource = window.readermdCurrentSplitSelection
              ? window.readermdCurrentSplitSelection()
              : null;
            ...
            return {
              ...
              mappedBlocks: document.querySelectorAll(
                "#preview-document [data-readermd-source-start]"
              ).length,
            };
            """,
    ...
    func testSplitSyncScrollsPreviewToTheMatchingSourceLine() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            """
            const applied = window.readermdApplySplitScrollPosition({
            ...
            return {
              ...
              target: Array.from(document.querySelectorAll(
                ...
                Number(element.dataset.readermdSourceStart) === targetLine
              ).map((element) => ({
                tag: element.tagName,
                top: element.getBoundingClientRect().top,
                start: element.dataset.readermdSourceStart,
                end: element.dataset.readermdSourceEnd,
              })),
              position: window.readermdCurrentSplitScrollPosition(),
            };
            """,
    ...
    func testSplitSyncRefreshesOffsetsAfterRichListEditing() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            return {
              markdown: window.readermdFlushEditor(),
              position: window.readermdCurrentSplitSelection(),
            };
            """,
            contentWorld: .page
        )
    ...
    func testEmptyLineShowsCompleteMarkdownInserter() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            selection.removeAllRanges();
            selection.addRange(range);
            window.readermdUpdateBlockInserter();
            return {
              visible: !document.querySelector("#block-inserter").hidden,
              blocks: window.readermdAvailableBlocks(),
            };
            """,
            contentWorld: .page
        )
    ...
    func testWideTableKeepsReadableColumnsAndExpandsIndependently() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            const serializedBefore = window.readermdSerializeEditor();
            const initial = {
            ...
            const serialized = window.readermdSerializeEditor();
            button.click();
    ...
    func testInserterProducesMarkdownForEveryBuiltInBlock() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            for (const action of actions) {
              await window.readermdRender(baseOptions);
              const paragraph = document.querySelector("#preview-document p");
              const range = document.createRange();
              range.selectNodeContents(paragraph);
              range.collapse(true);
              ...
              output[action] = window.readermdInsertBlock(action);
            }
            return output;
            """,
            contentWorld: .page
    ...
    func testRawMarkdownComposerInsertsArbitraryMultilineBlock() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            window.readermdInsertBlock("raw-markdown");
    ...
    func testExternalChangesHighlightAffectedBlocksAndNavigateBetweenThem() async throws {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            const targets = Array.from(
              article.querySelectorAll("[data-readermd-change-indexes]")
            );
            const initiallyActive = Array.from(
              article.querySelectorAll(".is-active-external-change")
            );
            const selectedCode = window.readermdSelectExternalChange(2, false);
            return {
              indexes: targets.map((target) => target.dataset.readermdChangeIndexes),
              kinds: targets.map((target) => target.dataset.readermdChangeKind),
    ...
    private func makeEditor(
        ...
        _ = try await webView.callAsyncJavaScript(
            """
            await window.readermdRender(options);
            return window.readermdSerializeEditor();
            """,
