(function () {
  ...
  function insertBlock(action) {
    ...
    if (action === "frontmatter") {
      ...
      const next =
        ...
        markdown.replace(/^\s+/, "");
      hideBlockInserter();
      if (window.readermdRefreshEditor) window.readermdRefreshEditor(next);
      return next;
    }
    if (action === "image") {
  ...
  function replaceEmptyLineWithMarkdown(target, snippet) {
    ...
    const markdown = flushEditor();
    if (window.readermdRefreshEditor) {
      window.readermdRefreshEditor(markdown);
    }
    return markdown;
  }
  ...
  function setEditable(value) {
    ...
    if (!editable) {
      imagePickerPending = false;
      if (window.ReaderMDDiagramEditor) window.ReaderMDDiagramEditor.close();
      hideToolbars();
      clearObjectSelection();
  ...
  function editObject(node, type) {
    if (type === "image") {
      const currentSource = window.readermdOriginalImageSource
        ? window.readermdOriginalImageSource(node)
      ...
      if (window.readermdSetImageSource) {
        window.readermdSetImageSource(node, source.trim());
      } else {
      ...
      scheduleChange(true, true);
    } else if (type === "diagram" && window.ReaderMDDiagramEditor) {
  ...
  function openDiagramEditor(node) {
    ...
    objectToolbar.hidden = true;
    window.ReaderMDDiagramEditor.open({
      card: node,
      source: source,
      onCancel: function () {
  ...
  function editTable(node, action) {
    ...
    if (window.readermdRefreshTableLayout) {
      window.readermdRefreshTableLayout();
    }
    scheduleChange(true, true);
  ...
  function serializeImage(node) {
    ...
    const source = window.readermdOriginalImageSource
      ? window.readermdOriginalImageSource(node)
      : node.getAttribute("src") || "";
    const title = node.getAttribute("title");
  ...
  function refreshSplitSourcePositions() {
    article
      .querySelectorAll(
        "[data-readermd-source-start][data-readermd-source-end]"
      )
      .forEach((element) => {
        delete element.dataset.readermdSourceStart;
        delete element.dataset.readermdSourceEnd;
      });
  ...
  function setSplitSourcePosition(element, sourceStart, sourceEnd, lineOffsets) {
    ...
    element.dataset.readermdSourceStart = String(startLine);
    element.dataset.readermdSourceEnd = String(endLine);
  }
  ...
  function splitSourceBounds(element, lineOffsets) {
    ...
    const startLine = Number(element.dataset.readermdSourceStart);
    const endLine = Number(element.dataset.readermdSourceEnd);
  ...
  function currentSplitScrollPosition() {
    ...
    const candidates = splitMappedElements()
      .map((element) => ({
        ...
        startLine: Number(element.dataset.readermdSourceStart),
        endLine: Number(element.dataset.readermdSourceEnd),
      }))
      .filter(
        (candidate) =>
  ...
  window.readermdSetEditable = setEditable;
  window.readermdEditorDidRender = editorDidRender;
  window.readermdFlushEditor = flushEditor;
  window.readermdSerializeEditor = serializeDocument;
  window.readermdSerializeFragment = serializeFragment;
  window.readermdInsertBlock = insertBlock;
  window.readermdInsertPickedImage = insertPickedImage;
  window.readermdCancelPickedImage = cancelPickedImage;
  window.readermdUpdateBlockInserter = updateBlockInserter;
  window.readermdCurrentSplitSelection = currentSplitSelection;
  window.readermdApplySplitSelection = applySplitSelection;
  window.readermdClearSplitSynchronization = clearSplitCounterpartIndicator;
  window.readermdCurrentSplitScrollPosition = currentSplitScrollPosition;
  window.readermdApplySplitScrollPosition = applySplitScrollPosition;
  window.readermdAvailableBlocks = function () {
    return Array.from(
      blockInserter.querySelectorAll("[data-insert-block]"),
      (item) => item.dataset.insertBlock
    );
  };
  window.readermdUndo = function () {
    if (editable) {
      document.execCommand("undo", false, null);
      scheduleChange(true, true);
    }
  };
  window.readermdRedo = function () {
    if (editable) {
